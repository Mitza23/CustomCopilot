...
st.set_page_config(page_title="Customization Interface")
...
def display_loaded_documents():
    st.subheader("Loaded Documents")
    if st.session_state["assistant"].vector_store is None:
        st.write("No documents loaded.")
    else:
        documents = st.session_state["assistant"].vector_store.get()
        sources = set(meta.get("source") for meta in documents["metadatas"] if "source" in meta)
